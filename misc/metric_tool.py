...
def calculate_kappa(confusion_matrix):
    ...
    # 计算观测一致性(po) - 对角线元素之和/总样本数
    po = np.diag(hist).sum() / total

    # 正确计算期望一致性(pe)
    # 每类的真实比例 (行和/总数)
    actual_props = hist.sum(axis=1) / total
    # 每类的预测比例 (列和/总数)
    pred_props = hist.sum(axis=0) / total
    # 期望一致性是这两组比例的乘积和
    pe = np.sum(actual_props * pred_props)
...
def cm2score(confusion_matrix):
    ...
    # 2. Kappa Coefficient
    # ---------------------------------------------------------------------- #
    kappa = calculate_kappa(hist)

    # ---------------------------------------------------------------------- #
    # 3. Frequency weighted Accuracy & Mean IoU
    # ---------------------------------------------------------------------- #
